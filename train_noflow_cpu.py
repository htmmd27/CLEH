...
H_PA, H_PC, H_PT, author_num, conf_num, term_num, paper_num = load_data.graph_pubmed()
# 构造训练矩阵 默认80%训练集
#todo 分别取训练集
...
H_EP_train = torch.eye(paper_num, dtype=torch.float32)
...
def loss_function(pos_EAE, pos_ETE, pos_ECE, latent_EA, latent_EC, latent_ET, tau):
    ...
    return lori_EAE + lori_ECE + lori_ETE

model = myModel_DBLP_noflow(author_num, conf_num, paper_num, term_num, paper_num, hidden_dim, hidden_dim1, hidden_dim2, hidden_dim3, d_out, flow_layers)
optimzer = torch.optim.Adam(model.parameters(), lr = Learning_Rate)
start_time = time.time()
#model.to(device)
for epoch in range(Epoch_num):
    EventA_embedding,EventC_embedding, EventT_embedding, H_mean_A, H_mean_C, H_mean_P, H_mean_T = model(H_EA_train, H_EC_train, H_EP_train, H_ET_train)
    loss = loss_function(pos_EAE, pos_ETE, pos_ECE, EventA_embedding, EventC_embedding, EventT_embedding, tau)
    optimzer.zero_grad()
    loss.backward()
    optimzer.step()
